...
> 状态：S0 基线与契约、S1 可信完成闭环已交付，S2–S6 待实施；见 [S0 验证报告](./s0-validation-report.md)和 [S1 验收报告](./s1-validation-report.md)。本文件不构成发布、生产操作或付费模型调用授权。
> 源码基线：`F:\Program\Agent`，`dev/1.0.0`，HEAD `8dcfa7f`；编制前工作区干净。
> 适用产品：当前统一桌面客户端及其本机 Coding 执行链。历史普通版和 Remote 安装包单独核对。
> S0 类型和生成校验见 [契约决议](./s0-contract-decisions.md)；S1 已将 RunOutcome、ExecutionResult 接入本机 API 和桌面结果投影，其他阶段能力仍为拟议设计。
...
S0、S1 已交付范围和复跑入口分别见 [S0 阶段交接](./s0-validation-report.md)、[S1 验收报告](./s1-validation-report.md)。当前达到 M0 的源码与隔离测试范围；下一阶段为 S2 项目指令与上下文，须由新的用户请求启动。原有受限工具、持续终端和恢复能力边界仍适用。
