...
> 状态：S0–S3 已接入本机主链，S4–S6 待实施；见 [S0 验证报告](./s0-validation-report.md)、[S1 验收报告](./s1-validation-report.md)、[S2 验收报告](./s2-validation-report.md)和 [S3 验收报告](./s3-validation-report.md)。本文件不构成发布、生产操作或付费模型调用授权。
...
> S0 类型和生成校验见 [契约决议](./s0-contract-decisions.md)；S1 接入 RunOutcome、ExecutionResult，S2 接入 ContextItem 与派生压缩检查点，S3 接入 WorkspaceIdentity、读取快照与补丁日志。S4–S6 能力仍为拟议设计。
...
| --- | --- | --- | --- |
| G01 | 原桌面 `AgentRuntime` 未传入完成验证器；S1 已接入 | 明确修改任务的零工具假完成负例已通过，不能仅凭 completed 宣称完成 | S1 已修复 |
| G02 | 原命令非零退出码仍包装为工具成功；S1 已分离进程与业务结果 | pytest 退出 1 反馈失败，搜索空结果按类别解释 | S1 已修复 |
| G03 | S2 已用有序 ContextItem 替代最近 12 条截断，保留工具关联及旧记录来源 | 原始必需输入超限时明确阻塞，不保证无限历史 | S2 隔离验证 |
| G04 | S2 已接入完整请求预算、输出预留与程序化压缩 | 字节估算与实测分开；真实模型质量待 S6 | S2 隔离验证 |
...
S0–S3 的范围和复跑入口见各阶段报告。S3 本机存储为 schema 5，下一阶段 S4 持续终端、流式输出与权限须由新的用户请求启动。当前仍是源码与隔离测试交付；持续终端、OS 隔离和自动恢复能力边界仍适用。
