...
> 状态：S0、S1、S2 已接入本机主链，S3–S6 待实施；见 [S0 验证报告](./s0-validation-report.md)、[S1 验收报告](./s1-validation-report.md)和 [S2 验收报告](./s2-validation-report.md)。本文件不构成发布、生产操作或付费模型调用授权。
...
> S0 类型和生成校验见 [契约决议](./s0-contract-decisions.md)；S1 接入 RunOutcome、ExecutionResult，S2 接入 ContextItem 与派生压缩检查点，后续阶段能力仍为拟议设计。
...
S0–S2 的范围和复跑入口见各阶段报告。S2 存储契约已落地，下一阶段 S3 仓库检索与多文件编辑须由新的用户请求启动。当前仍是源码与隔离测试交付；原有受限工具、持续终端、OS 隔离和自动恢复能力边界仍适用。
