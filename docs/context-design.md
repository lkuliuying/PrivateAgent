...
> **2026-09-08 S3 补记**：本机数据库当前为 schema 5。版本化读取的正文超过 1200 字符时，请求投影保留版本/范围元数据、1200 字符摘录和原始 ContextItem 引用；完整工具结果继续保存，不代表已向模型展示全部正文。模型可通过 `read_context_content` 续读原结果，或使用 `read_code_file` 的行/列及 `expected_version` 定位原文件。多文件补丁按各目标目录注入受信规则，首次尚未交付的规则阻断写入并要求下一轮检查；规则变化仍使活动任务失效。见 [S3 验收报告](analysis/coding-agent-upgrade-20260908/s3-validation-report.md)。
...
- `read_context_content` 只接受当前会话的 item ID，最多续读 6000 字符。一般大工具结果在请求中只保留 3000 字符摘录与引用；S3 版本化读取采用上文的 1200 字符专用投影。单项保存最多 2 MiB、单次历史读取最多 20000 条或 64 MiB；超限明确停止。引用覆盖真实工具返回，旧文件工具历史中已经裁掉的尾部无法恢复；新工具可按行和列重新读取并核对版本。
...
- 新运行可设 `context_limits.auto_compact=false` 停止自动压缩；原始必需输入过大时仍阻断，不静默截断约束。程序回退必须理解当前 schema 5，禁止删表或覆盖数据库降级。S2 历史字段、测试及限制见 S2 报告，后续调整见 S3 报告。
