...
- ✅ **发布级自动化验证**：2026-07-26 的 `release-check-full.bat` 已完成 11/11 门禁：后端 `pytest` 399 项、前端 Vitest 140 项、Playwright Chromium E2E 27 项、Rust 单测 2 项全部通过，生产构建满足包体预算，`alembic current -> 0012 (head)`，诊断包逐成员脱敏与 updater 清单校验通过。证据输出为 `dist/release-check-0.1.2.json` 与 `.md`。
