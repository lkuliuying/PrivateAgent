...
>   `PA_TOKEN_ESTIMATE_SAFETY_FACTOR=2.0` 后 5/5 ≥ 真实值
...
> - **R3 灰度开启（2026-08-06 三轮，用户授权）**：按决策报告（§6）开启批 A 5 个开关
>   （Agent API / 只读工具 / ContextBuilder / 输出验证 / RAG 工具），聊天接管与摘要 worker 保持关闭；
>   只读 smoke 验证 capabilities 与路由；首个真实遥测窗口已产生（legacy 归零判定链路工作，
>   含零调用基线行修复与测试）。遥测窗口自 2026-08-06 08:50 起积累。
...
- `docs/archive/planning/remaining-work-plan.md` 的基线仍写“只有 0.1.2 报告”。
...
### 当前 Windows `0.2.1` 稳定化候选放行（路线图阶段 A–C，`docs/archive/planning/project-roadmap-20260806.md`）
