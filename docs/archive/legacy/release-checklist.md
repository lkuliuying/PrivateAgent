...
## 统一 Coding S6 交付阻断项（2026-09-09）

此节是当前 Coding 专项的补充，下面 0.2.1 等历史成绩不能替代。S6 工具开发及公开校准通过后仍不得自动放行 M3。

- 候选清单同时记录 UI、本机运行时、共享核心、exec-host、schema 7、执行/恢复协议 1.0 和最低代理流能力；比较实际文件摘要与源码 manifest，另核对安装进程实际路径。
- S6-T01–T12 逐项记录通过、失败、未执行或有依据的不适用；局部测试只作为对应门禁的部分证据。
- 默认模型分别完成固定 30 题 × 3 次，编码目标预先固定 81 次分母，授权拒绝的 9 次只统计系统行为。预算、失败与人工介入不能删去；公开校准参考题需要换成独立的保留题后才能形成盲评结论。
- 在声明支持的 Windows 版本/架构、标准用户、空格和中文路径的干净环境中安装，不借用源码目录或开发机虚拟环境。验证供应商失败、依赖缺失、退出及重启。
- 对独立旧数据副本执行升级，并用确切旧程序验证程序回退；合成 schema 迁移测试不能代替实际旧安装包。
- 界面输出延迟、600 秒命令、进程树取消和权限正反对照的证据要绑定候选；不把历史 S5 数字当作新的测量。
- 使用/API/安全/排障/升级文档应与声明能力一致。S6 生成“继续验收”的决议时保留阻断项；发布、签名与远端修改仍按对应明确任务执行。

本次开发及实际验证见 [S6 报告](../../analysis/coding-agent-upgrade-20260908/s6-validation-report.md)。
...
- 安装包未代码签名：SmartScreen 拦截，需手动绕过（见 `docs/archive/legacy/signing-and-keys.md` §2）。
- macOS / Linux 仅有构建脚本与差异清单，未实测（见 `docs/archive/legacy/cross-platform.md`）。
