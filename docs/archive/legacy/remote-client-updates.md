...
远程安装版支持“检查更新 → 确认下载并安装 → 重新启动”。2026-09-19 起当前桌面源码仅保留 API Key 和本机执行器，不依赖平台账号服务；更新协议、应用标识和渠道隔离保持不变。本说明不表示新版本已打包或发布。
...
| 默认清单地址 | `https://www.liuyingapi.top/updates/remote/latest.json`（更新地址独立于模型供应商） |
| 本地后端 | 1.0.3 起捆绑轻量本机执行器，项目和工具在本机运行；不启动普通版完整后端，也不执行停止普通版 sidecar 的安装钩子 |

1.0.3 的历史账号接口与部署方式见 [联网客户端部署指南](./connected-desktop-rollout.md)。当前使用方式以[本机模型直连说明](../../direct-model-execution.md)为准；项目文件不自动跨电脑同步，模型请求从本机发送给用户配置的供应商。

本地安装版的标识、发布工作流、更新地址不变。不要把远程包覆盖到现有通用 GitHub `latest.json`：旧客户端使用相同默认平台键，无法安全区分本地版与远程便携版。

以前发送的便携 EXE 需要先手动安装一次远程安装版，并从新快捷方式启动；关闭旧便携进程，不再运行旧 EXE。由于远程安装版使用独立应用标识，首次可能需要重新设置客户端偏好和本机模型供应商；当前不再提供平台登录。旧服务器账号及历史数据不因安装包替换而删除。之后远程安装版之间的更新保持相同应用标识及数据目录，不需要卸载。

## 构建与验证

在已有 Node、Rust、MSVC 和桌面依赖的 Windows 开发环境中，从仓库根目录执行。命令中的更新域名和版本均为示例，不再接受平台 API 源站位置参数；每次正式发布必须使用比已发布版本更大的 `major.minor.patch` 版本号，不修改或复用已发布的版本目录。

先预览配置，不构建、不访问签名材料、不上传：

```powershell
.\scripts\build-remote-client.cmd --release --version 1.0.1 --dry-run
```

不使用签名密钥的安装包验证：

```powershell
.\scripts\build-remote-client.cmd --preview-installer --version 1.0.1
```
...
```powershell
.\scripts\build-remote-client.cmd --release --version 1.0.1
```

也可以独立托管下载服务：

```powershell
.\scripts\build-remote-client.cmd --release --version 1.0.1 --update-url "https://downloads.example.com/remote/latest.json" --download-base-url "https://downloads.example.com/remote"
```
...
```text
build-info.json                 # 版本、提交、是否有未提交修改、API Key 模式与更新地址
...
5. 升级后自动进入本机工作区，版本已变化；本机项目、模型配置与客户端偏好按预期保留，未访问平台登录接口。
