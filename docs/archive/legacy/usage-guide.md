...
## 统一客户端 Coding 使用补充（S6，2026-09-09）

以下适用于统一客户端的本机 Coding 链，后文历史版本的业务阶段编号与 Coding 专项 S0–S6 不同。当前开发和验收范围见 [S6 开发报告](../../analysis/coding-agent-upgrade-20260908/s6-validation-report.md)，不以历史 M3 字样表示当前 Coding 已正式验收。

1. 在 Coding 工作台选择本机项目，确认模型与项目开发工具可用。项目规则需要显式信任；规则内容不会增加文件、命令或网络权限。
2. 提出目标及允许修改范围，在审批面板检查文件 diff 和命令。可信项目命令以当前用户身份运行，批准前确认其项目外文件及网络范围。
3. 终端出现首段输出仍可能继续运行。续读输出并检查退出码；“已结束”与“已验证完成”不同。旧记录没有验证字段时保持“未记录目标验证”。
4. 运行中可以追加收紧约束、暂停和取消。暂停会使旧审批失效；重启后的继续操作先核对现场，可能建立关联新运行。未知副作用不能自动重放。
5. 在审查面板区分任务补丁、初始用户改动和命令候选变化。外部编辑导致摘要改变时，重新审查，避免强制回退。

源码与回环验收不构成真实模型质量、干净安装或升级承诺。项目自身需要的 Python、Node、Rust/MSVC 等开发工具也不因客户端携带本机运行时而自动具备。

> 版本：v0.1 · 日期：2026-07-04
> 对应需求：`docs/archive/legacy/requirements.md` v0.1 · 对应计划：`docs/archive/phases/phase1-plan.md` v0.3
...
第四阶段把产品升级为长期个人系统。导航扩展为七入口：聊天 / 知识库 / 项目 / 学习 / 任务 / 记忆 / 设置。

- **长期记忆**：记忆页支持创建、搜索、编辑、禁用、删除；聊天可检索已启用且非敏感的记忆，并在回答结束时返回引用来源。
- **学习复习**：学习页支持今日复习、卡片评分、错题本、主题 dashboard 与学习周报；复习结果可生成候选记忆。
- **文档集合与抽取**：知识库页可创建文档集合，执行术语、行动项、关键观点、表格摘要、代码片段抽取，并按模板生成 Markdown 报告。
- **编码工作流 2.0**：项目页支持命令配置、多文件 patch set、审批写入、拒绝、回滚和命令失败诊断。
- **任务计划 2.0**：任务页可生成计划草稿、编辑步骤 JSON、批准整体计划、暂停/取消/继续，并可从指定步骤恢复；证据视图支持筛选。
- **Provider 与备份**：设置页支持 Ollama / OpenAI-compatible / Claude Provider 选择，远程 Provider 默认关闭并展示发送范围；支持创建备份包和恢复预览。
...
第五阶段已落地完整发布工程化链路（Windows 硬验收）：可复现构建脚本（去硬编码路径）、`release-check.bat` 发布前校验、`generate_release_manifest.py` 发布清单、`generate-latest-json.py` 自动产出 updater 清单、`UpdateChecker.vue` 分类错误提示、`signing-and-keys.md` 签名与密钥治理、`release-checklist.md` 发布 QA 矩阵、`measure_sidecar_baseline.py` 体积与启动基线、onedir 评估 spec、`build-sidecar.sh` 跨平台脚本、`cross-platform.md` 预研。详见 `docs/archive/phases/phase5-plan.md`、`docs/archive/phases/phase5-requirements.md`、`docs/archive/legacy/release-checklist.md`、`docs/archive/legacy/signing-and-keys.md`、`docs/archive/legacy/cross-platform.md`。
...
2. **Windows 代码签名**：当前无证书，安装包未签名，SmartScreen 会警告（绕过：更多信息 -> 仍要运行）。方案见 `docs/archive/legacy/signing-and-keys.md` §2。
3. **macOS / Linux 实机构建与 smoke**：见 `docs/archive/legacy/cross-platform.md`（当前仅预研，未实测）。
