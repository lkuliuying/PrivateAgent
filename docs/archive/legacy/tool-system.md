...
> 适用边界：本文描述 `personal_assistant` 完整后端工具链。2026-09-19 桌面轻量执行器的统一契约、原生 Git、有界并行和技术文档 MCP 见[本机 Agent 工具系统](../../local-tool-system.md)。两套运行入口不能混用。
...
完整 MCP 边界见 `docs/archive/legacy/mcp-design.md`。
...
覆盖输入/输出校验、远程 Schema 引用拒绝、参数替换、token 重放、并发消费、结果脱敏、大小限制、claim 冲突和审批恢复。上线与回滚顺序见 `docs/archive/legacy/deployment-guide.md`。
