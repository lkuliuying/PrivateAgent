...
> **历史执行台账（不得当作当前状态）**（2026-08-05 补注，2026-08-06 复核）：本文是 2026-08-02 对变更前基线 `19ea237` 的历史审计快照；文中「当前数据库 `0012 (head)`」「应用主库仍为 `0012`」等描述只反映该审计时间点，属于历史证据。此后应用主库已获授权迁移到 `0020 (head)`（2026-08-05，48 张原表行数零变化，回滚克隆 `personal_assistant_preupgrade_20260805111304` 保留），versioned RAG indexing/retrieval 已生产启用；Agent Runtime、MCP、自动摘要仍保持默认关闭。最新状态以 `docs/archive/legacy/agent-runtime.md`、`docs/archive/legacy/rag-design.md`、`docs/archive/legacy/database-design.md`、`docs/archive/planning/migration-plan.md`、`docs/testing-guide.md` 顶部「当前状态（2026-08-06）」摘要为准。
...
项目结构、运行方式、关键依赖、核心数据流、数据库、最严重风险和处理清单已完成审计。下一步按照 `docs/archive/planning/migration-plan.md`：
...
完整设计、回滚和限制见 `docs/archive/legacy/mcp-design.md`。
...
- 新增三项克隆边界单元测试后，Python 全量为 403 passed。完整操作与回滚步骤见 `docs/archive/legacy/database-upgrade-runbook.md`。
...
- 执行 notebook 位于 `docs/archive/legacy/analysis/rag-data-quality-audit-20260802.ipynb`，4 个代码单元从头执行且无 error output。报告 artifact 通过 Data Analytics MCP validator（6 datasets / 3 sources / 2 native charts / 2 native tables）并成功 render；独立验证记录见 `docs/archive/legacy/analysis/rag-data-quality-validation.md`。
