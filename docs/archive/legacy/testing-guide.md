...
> 整理前的历史测试指南；旧服务器、数据库和 RAG 命令已退役。当前命令见[本机测试指南](../../testing-guide.md)。

## 当前本机验证入口（2026-09-20）

旧服务端及其专属测试已删除。使用 `.venv\Scripts\python.exe -B scripts/run_coding_validation.py --suite all` 运行保留的本机套件；`context-alignment` 覆盖新摘要、预算和发行边界，`shared-models` 覆盖迁移到共享包的模型协议，`desktop-packaging` 覆盖安装与签名辅助。协议检查为 `scripts/protocol_codegen.py --check`，不再生成服务端 DTO。

前端执行 `npm test --prefix apps/desktop` 和 `npm run build --prefix apps/desktop`；在 MSVC 开发环境执行 `cargo test --offline --manifest-path apps/desktop/src-tauri/Cargo.toml --lib`。构建使用 `scripts\build-client.cmd`，不依赖 MySQL 或数据库迁移。当前实测结果与未通过项见[本次验证记录](../../solutions/2026-09-20-local-only-context.md)。下文旧数据库、RAG、迁移、`release-check-full` 等入口仅保留当时记录，不适用于当前源码。
...
> **S2 项目指令与上下文**：`.venv/Scripts/python.exe -B scripts/run_coding_validation.py --suite context` 运行规则、历史与压缩边界，已并入 `all`。旧服务端入口 `run_coding_legacy_validation.py` 已删除，共享模型测试现归 `shared-models`。前端覆盖 `LocalContextPanel.spec.ts`。历史成绩见 [S2 验收报告](../../analysis/coding-agent-upgrade-20260908/s2-validation-report.md)。
...
```powershell
uv run pytest -q tests/unit/test_model_gateway.py tests/unit/test_agent_runtime.py `
...
发布门禁的诊断脱敏 smoke 必须使用 `resolve_test_database_url`；不得直接连接 `PA_DB_URL`。诊断健康聚合也必须注入该测试 session，不能通过模块级 engine 旁路到主库。本轮修复前曾向主库留下两条 `diagnostic_runs` 审计记录，现已在 `docs/archive/legacy/database-design.md` 记录并保留。修复后的最终复跑没有新增主库或测试库行。
