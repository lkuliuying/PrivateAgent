...
> **2026-09-20 更新**：当前仅保留 API Key 本机桌面源码，已删除本文旧完整后端、MySQL、Alembic 和服务器部署入口。新电脑按[主 README 的开发与构建步骤](../../../README.md)恢复依赖并验证；Python 使用 `uv sync --locked --all-extras`，前端使用 `npm ci --prefix apps/desktop`，完整本机验证包使用 `scripts\build-client.cmd`。以下 2026-09-03 的提交、远程引用和旧服务端步骤仅作为历史交接记录，不是当前要求。未提交改动仍须由用户自行决定如何同步，本次没有执行提交或推送。
...
- 联网版 `PrivateAgentRemote` 在用户电脑运行轻量本机执行器，项目文件与执行记录留在本机，不依赖平台账号，模型请求直接从本机发送到配置的供应商。它不是“把 Windows 项目目录交给 Linux 服务器执行”。
...
git diff --stat
git diff -- README.md
git diff -- docs/archive/legacy/new-computer-development.md
git diff --staged
```
...
```powershell
git add -- docs/archive/legacy/new-computer-development.md
...
在新电脑使用自己的 GitHub 登录和开发凭据，在应用本机设置模型供应商。不要复制旧电脑的 GitHub token、SSH 私钥、Windows 凭据管理器内容、登录会话、生产环境文件或签名私钥。
...
访问 `http://127.0.0.1:1420/?workspace-preview=running`。该入口使用开发预览数据，不等于真实 Tauri、本机执行器或生产模型验收。
...
当前构建不接收平台 API origin，也不注入平台令牌；示例版本仅用于 dry-run，应按实际版本规则构建。真正构建、签名、上传和发布是独立操作，按 [`docs/archive/legacy/remote-client-updates.md`](./remote-client-updates.md) 执行，不要仅凭 dry-run 宣称安装包已生成或已发布。
...
浏览器模拟测试通过不等于真实 Tauri 安装、自动更新、本机会话恢复、付费模型调用或服务器部署验收通过。
...
| 内容 | 处理方式 |
| --- | --- |
| `.env`、`.env.local`、`.env.container`、`.secrets/` | 在新电脑按示例重新创建，只填写新环境自己的值 |
...
| Windows 凭据管理器、GitHub 凭据、模型密钥 | 在新电脑分别配置，不导出旧凭据 |
