...
生产 rollout 已完成（2026-08-05，详情见 `docs/archive/legacy/analysis/rag-data-quality-validation.md` 与 `docs/archive/legacy/rag-design.md` §7）：主库 `0020` 上 `migrate_versioned_rag.py` 对 4 个 canonical 文档小批构建成功（4/4，来源哈希一致），`PA_VERSIONED_RAG_INDEXING_ENABLED=true` 与 `PA_VERSIONED_RAG_RETRIEVAL_ENABLED=true` 均已启用，生产 hybrid 评测 10 个 reviewed case 全部通过。
