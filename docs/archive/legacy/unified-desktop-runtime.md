...
> 历史归档（2026-09-20 整理）：保留原有设计、操作和验收记录；其中已退役的服务端、旧页面和脚本不适用于当前源码。当前入口见[文档中心](../../README.md)。

> **2026-09-09 S5 收尾**：全部适用 S0–S5 阻断门禁已通过，当前 Windows QA 范围满足 M1/M2，可进入 S6。普通 ConPTY、真实桌面追加约束/暂停/继续/重启/关联恢复/审查、600 秒命令以及真实符号链接拒绝均已验证。最终实际安装的前台 120 条中文样本 p95 为 386.70 ms，最大 5034.10 ms；输出区域只在用户位于尾部时跟随内外层滚动，长恢复审查面板限制高度且可滚动。最新 636 文件源码摘要、4 个构建产物和 4 个安装文件严格核对通过；临时模型及服务已清理，原测试文件保留，沙箱租约 0。见 [S5 剩余门禁验收记录](../../analysis/coding-agent-upgrade-20260908/s5-remaining-gates-validation-report.md)。受限 PTY 仍明确拒绝；严格工具目录和 Node 入口限制保留。本轮仅本地提交 S5 后停止，S6 真实模型质量及完整交付演练未开展。下列早期“未放行”等结论保留历史，不代表当前门禁状态。

> **2026-09-09 S5 阻断补充**：原生 AppContainer 文件/网络正反门禁已通过。最终源码完整回归为 343 passed、2 skipped、0 xfail；跳过原因为 ConPTY 环境探针失败和真实符号链接权限不足。每次执行使用独立身份与可核对清理日志；可写工作区拒绝硬链接，受限 PowerShell 使用专用 PSDrive。历史交换支持 schema 4–7 的基础记录投影，不备份恢复授权。当前匹配源码的独立候选包已构建、安装并核对副本；真实 Tauri 强退、重启、关联恢复、父子进程回收、外部修改保护及实际回滚已有证据。临时模型已删除并重载确认，回环服务已停止。严格沙箱仍限制大工具目录和默认 Node 入口解析，未通过扩大祖先权限解决。Nginx HTTP 8081 返回 308、HTTPS 443 返回 401，真实 WebView 匿名 HTTPS 请求无网络错误。当前证据与源码/产物摘要见 [S5 阻断项补充验收](../../analysis/coding-agent-upgrade-20260908/s5-blockers-validation-report.md)。完整运行中协作组合、PTY、性能和其余交付门禁未全部完成，S6 尚未开发；后续历史段落中的“无文件/网络沙箱”描述不代表本轮实现。

> **2026-09-09 S5 恢复与协作**：恢复契约 1.0 增加安全边界暂停、持久化追加约束、关联新运行继续、工作区排队与显式 worktree。SQLite 当前为 **7**，升级先备份；旧终态和事件保留，旧运行不冒充精确恢复。重启后的命令身份或结果不明时阻止继续，不凭 PID 附着、不自动重放副作用。详见 [S5 开发与验证报告](../../analysis/coding-agent-upgrade-20260908/s5-validation-report.md)。以下各阶段日期段落保留当时证据；安装升级、完整桌面验收与 M1/M2 仍未放行。

> **2026-09-08 S4 执行与流式**：已接入持续执行、stdin、滚动输出、会话服务关闭及模型公开增量。SQLite 当前为 **6**，旧 schema 2/3/4/5 先备份再事务升级。新桌面以执行契约 1.0 请求新工具；命令和 stdin 必须逐次批准可信项目的当前用户文件/网络范围，四种权限均不提供管理员或文件/网络沙箱能力。缺少新宿主时拒绝新可写运行，仍可只读。旧客户端省略契约时保留旧工具语义。原生隔离、PTY、真实安装链、Provider 与 UI p95 未完成完整验收；本轮未安装或部署。详见 [S4 开发与验证报告](../../analysis/coding-agent-upgrade-20260908/s4-validation-report.md)。下文历史版本数字以各阶段日期解释。
...
> **2026-09-08 S1 完成验证**：本机 Coding 主链已接入输出验证器，区分已回答、已验证、未满足、受阻和未知；详见 [S1 验收报告](../../analysis/coding-agent-upgrade-20260908/s1-validation-report.md)。`completed` 仅表示运行结束，旧记录缺少证据时展示未确认结果。最低验收来自用户请求和受审查规则，在任务正文可见，不能增加权限。文件要求核对磁盘摘要，命令结果保留真实退出及输出引用；测试证据受有界工作区摘要和变更版本约束。失败最多纠偏两次，沿用同一预算；拒绝及未知副作用阻止同范围绕行。结果、最终消息和终态沿用现有事务提交，SQLite 仍为 3。人工条件保留未验证；本轮没有打包、覆盖安装或扩展终端与 OS 隔离能力。
...
服务器业务后端提供账号、管理、知识库、记忆等能力；这些服务的 MySQL/ChromaDB 数据不在本次 SQLite 迁移范围内。客户端不再启动完整本机业务后端；选择本机模型只替换 Coding 推理服务，不改变账号或业务接口。工作树创建仍未移植；上下文自动/手动压缩已由 S2 接入，能力位与本机接口一致。
...
当前 SQLite schema 为 7：除项目、会话、运行、审批、授权、事件、有序上下文与压缩检查点外，S3 增加读取快照和补丁日志，S4 增加受管理执行及滚动输出，S5 增加运行恢复检查点、控制请求及工作区租约。采用 WAL、foreign_keys、5 秒 busy timeout 和 FULL 同步。较大的内容存入同目录 `artifacts/<SHA-256>`，读取时校验长度及摘要。SQLite 不加密；账号目录是应用层隔离，不阻止同一系统用户自行读取磁盘。不要把它当作多用户操作系统安全隔离。

旧轻量 `objects/runs` 表在首次打开时事务迁移，原始表保留为 legacy 表；schema 2/3/4/5/6 升级到 7 同样先生成备份，命名为 `*.pre-v7-<唯一标识>.sqlite3`，实际原版本以备份内的 `PRAGMA user_version` 为准。迁移记录包含备份摘要与完整性检查结果。原始消息和授权不被压缩重写；旧版本程序必须先核对 schema 兼容性，不能删除新表降级。

启动恢复先取得本机数据库所有权锁。恢复契约 1.0 的活动运行变为 `interrupted`，旧协议仍变为失败；待审批记录取消，执行中的命令记为结果未知，完全访问授权撤销。界面核对检查点、文件事实、模型能力和剩余预算后，暂停中的任务使用原 ID 继续，中断/取消的任务创建关联的新运行；原事件和终态不改写。继续原完全访问任务须明确确认当前会话的新授权，旧批准不复活。身份或结果不明的命令及未核实写入阻止继续，也阻止新任务绕过工作区租约。

新协议以 Git 工作树顶层及实际目录身份协调：同一工作树最多一个可变更任务，账号最多两个工作区并发、十六个活动或排队任务。不同 worktree 可独立运行；暂停保留租约，排队可暂停或取消。项目选择不会结束其他工作区的新协议普通任务；完全访问授权仍随项目切换撤销。保留服务继续持有原工作区，关闭后才释放。旧协议保留账号单任务和项目切换行为。

“独立工作区”入口从用户选定 ref 的提交创建 detached worktree，不带入未提交内容。清理仅针对本应用拥有的工作树；有修改、未跟踪/忽略文件、后续提交、活动任务或身份不符时拒绝清理。恢复、控制与审查的限制及复跑方法见 S5 报告，不直接对生产数据套用测试命令。
...
| --- | --- | --- |
| 只读 | 拒绝 | 拒绝 |
| 总是询问 | 新增或修改前逐次预览、批准 | 新持续命令与 stdin 逐次审批；受控 PowerShell 只读 cmdlet 仍逐次批准 |
| 替我批准 | 项目内新增或修改默认自动批准并审计；模型可主动要求批准 | 新持续命令与 stdin 仍逐次审批；登记的只读 PowerShell 可自动批准 |
| 完全访问 | 限时授权内自动批准项目目录中的登记文件操作 | 新持续命令与 stdin 仍逐次审批且受授权期限限制；拒绝直接 shell 拼接、内联求值和提权入口 |

上表描述携带执行契约 1.0 的新桌面链路。旧客户端的 `run_project_command` 保留历史自动批准行为，仅用于兼容；新运行不暴露该入口。审批的项目内 cwd/argv 约束不能限制项目脚本的间接系统访问；可信执行预览明确声明当前系统用户可读写的项目外文件和网络范围。
...
容量来自当前模型配置；最近一次供应商 input tokens 与 S2 完整请求的保守字节估算分别展示，不能把估算或累计计费量当成当前精确占用。未知容量显示未知，预算器只允许有明确内部上限的小请求。缓存命中率仍为当前会话、当前模型的 `累计 cached_tokens / 累计 input_tokens × 100%`；缺少缓存用量、无效或零输入不计入，无有效数据时显示 `--`。旧记录仅使用仍保留的真实用量，不推算丢失历史。预算和版本字段保存在运行 JSON，原始上下文及派生检查点使用 schema 4 的独立表。
...
| 新增 | `docs/archive/legacy/unified-desktop-runtime.md` |
