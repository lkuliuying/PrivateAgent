...
> **2026-09-20 适用范围更新**：下文为 2026-08-02 的旧架构历史，相关服务端已退役。当前是 Tauri/Vue → stdio 本机执行器 → 共享 AgentRuntime/模型供应商，数据保存为本机 SQLite，工具由本机 exec-host 执行。现行模块边界见[主 README](../../../README.md)及[本机化记录](../../solutions/2026-09-20-local-only-context.md)。
...
容器 API 启动前等待独立 MySQL 并执行 Alembic；任何迁移失败都会拒绝提供可写服务。命名卷不是备份，桌面主库和容器新库不得通过同名数据库或未审计挂载混用。完整运行和回滚边界见 `docs/archive/legacy/deployment-guide.md` §8。
