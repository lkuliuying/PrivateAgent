...
> 本文保留安装副本的应急修复步骤。若要改为“开发机推送、服务器拉取后重启”，请按[源码更新流程](./server-code-update-workflow.md)先整理服务器历史、核对完整运行包，再切换启动入口；完成切换后，日常更新不再使用此五文件复制工具。尚未完成服务器切换时，不要仅凭文档存在就跳过当前运行包修复。
...
本次使用 repair-connected-runtime.py（历史路径：`scripts/repair-connected-runtime.py`，已移除） 对已核对的五个运行包文件做定向修复。
