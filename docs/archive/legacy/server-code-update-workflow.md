...
> 2026-09-19 更新：平台登录、注册、账号管理和管理员日志接口已从当前源码移除。本文涉及这些功能及 1.0.3 补丁的步骤仅作为历史记录；当前使用与兼容边界见[API Key 单模式说明](../../direct-model-execution.md)。本次变更未部署服务器。
...
- 源码启动入口（历史路径：`scripts/start-connected-server.py`，已移除）：选择当前仓库的包，保留原服务器入口；附只读入口检查和完整 Python 文件摘要比较。
- 一键更新工具（历史路径：`scripts/update-connected-server.py`，已移除）：默认一次执行，源码备份与阶段记录，按差异决定是否重启；保留 check/apply，保护本地修改和分叉历史，不改外部配置、不装依赖、不操作其他服务。
- 隔离回归测试（历史路径：`tests/unit/test_connected_server_workflow.py`，已移除）：验证临时 Git 远端、真实快进和拒绝条件；Supervisor 使用替身，不等于 Linux 生产演练。
