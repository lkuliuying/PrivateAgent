...
- **多平台 updater 清单**：`scripts/generate-latest-json.py` 支持 `--extra-platform KEY:INSTALLER_PATH`（可重复），为 darwin-aarch64 / darwin-x86_64 / linux-x86_64 生成独立 `{signature, url}` 条目；多平台清单逻辑有测试（`tests/packaging/test_release_manifest.py`）。
...
- Windows 是第五阶段硬验收目标，已完成（见 `docs/archive/phases/phase5-plan.md` M1–M5 与 `docs/archive/legacy/release-checklist.md`）。
