...
> 支持两种部署模型：Windows 桌面应用 + 本地 Python sidecar，以及由服务令牌保护的独立后端 API。服务器模式仍只让后端监听 loopback，由外部 HTTPS Nginx 或容器反向代理提供公网入口；两种模式不共享秘密存储或生命周期假设。
...
详细签名与发布步骤见 `docs/archive/legacy/release-checklist.md` 和 `docs/archive/legacy/signing-and-keys.md`。
...
正式升级前按 `docs/archive/legacy/database-upgrade-runbook.md`：
...
2026-09-19 起已删除平台登录、注册、账号管理和注册邮件服务。独立后端的 `PA_API_TOKEN` / `PA_API_TOKEN_FILE`、Host/Origin 校验及审计继续有效；旧登录会话不能替代服务令牌。不要关闭鉴权来恢复旧客户端兼容。

当前桌面通过本机执行器直接访问所配置的模型供应商，无需服务器源站构建参数或注册账号；见[本机模型直连说明](../../direct-model-execution.md)。服务令牌不可注入桌面包，模型供应商 API Key 也不能用于访问独立后端。

历史数据库表、外键和迁移未删除，不新增清空迁移，不自动认领历史 owner。应用不再读取 `smtp.env`，原秘密文件不会被自动删除。

所有 HTTP 操作继续写入 `audit_logs`，不保存请求正文和秘密。数据库审计按 `PA_AUDIT_LOG_RETENTION_DAYS` 清理，文件日志按 `PA_LOG_RETENTION_DAYS` 清理；账号会话与验证码不再被后台任务清理。旧管理员日志 API 已移除。

以下是早期 Compose 验证记录，不能作为本次源码已经部署的证据：
...
Windows 10/11 x64 是当前硬验收平台。macOS/Linux 有数据目录适配、sidecar 构建脚本和发布清单结构，但尚未实机构建、签名和 smoke；不得宣称正式支持。详见 `docs/archive/legacy/cross-platform.md`。

## 10. 观测与故障排查

运行期查看 `/health`、诊断中心、结构化日志、Agent run/events、MCP call log 和版本化索引状态。日志和报告不得包含密码、token、文档正文或完整模型提示。

常见故障和安全处理见 `docs/archive/legacy/troubleshooting.md`；API 入口见 `docs/archive/legacy/api-reference.md`。
