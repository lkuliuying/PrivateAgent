...
## 统一 Coding 与 S6 评测边界（2026-09-09）

统一客户端本机工作记录使用按账号隔离的 SQLite，默认私有 IPC；服务器只提供账号和模型配置/推理。后文完整业务后端的数据库、动态端口与认证说明不能直接套用于轻量本机服务。

受限 Windows argv 使用工作区 AppContainer、工具目录只读和禁网；受限 PTY 明确受阻。可信项目执行仍逐次批准，以当前用户权限访问文件和网络。初始 dirty、预览至写入期间的并发变化以及未知副作用分别由摘要、补丁日志和恢复租约保护；这些机制不替代 OS 隔离。

S6 运行器仅操作新建的原创合成项目。回环账号令牌只在进程内存和私有通信中使用，不写证据。运行器只批准固定可编辑路径以及任务预先指定的验证命令；其中验证命令采用显式的可信项目执行，不将其描述为恶意代码沙箱。Python、Vue 编译器和 Rust/MSVC 是已安装的开发依赖；隐藏断言在候选项目外执行，但公开参考实现及当前用户文件权限意味着它们不是防作弊盲评环境。

真实质量入口只接受明确声明不计费的字面回环模型配置，不接受密钥、远程地址或环境中的供应商凭据。用量缺失标 unknown，不填零成本。源码、命令输出和事件采用有界证据与字段脱敏；不能把任意私人项目或生产日志导入校准集。详见 [评测协议](../../analysis/coding-agent-upgrade-20260908/s6-acceptance-protocol.md)。
...
签名私钥、PFX 和密码文件遵循 `docs/archive/legacy/signing-and-keys.md`，不得进入仓库或发布包。
...
详细规则见 `docs/archive/legacy/tool-system.md`。
