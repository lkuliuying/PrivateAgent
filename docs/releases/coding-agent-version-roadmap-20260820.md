...
| `v1.0.0` | 稳定发布 | 契约冻结、旧执行链隔离、升级回退、文档与正式发布 | 7–10 个工作日 + 14 天 RC 观察 |

总开发量约 43–64 个工作日。单人全职预计 9–13 周；两人按后端执行链与桌面工作台拆分，预计 6–9 周。观察时间不计入开发工作日。
...
- `v1.0.0` 不增加新工具或大功能，只完成旧链隔离、硬化、迁移、文档和发布。
...
| `v1.0.0` | beta.1 → beta.2 → rc.1 → stable | 14 天 | 契约冻结、升级回退与正式发布完成 |
...
- [v1.0.0：稳定发布](./v1.0.0/v1.0.0-development-plan-20260820.md)

## 8. 路线完成定义

`v1.0.0` 发布后应同时满足：

1. 产品主线为 Project → Workspace → Thread → Run。
2. 前端不参与 Agent 决策循环。
3. 计划、工具、审批、验证和 Artifact 均来自 durable 事实源。
4. Coding Agent 可以读取、修改、验证并报告一个真实项目任务。
5. 权限、模型、Git、路径和进程边界可审计、可取消、可恢复。
