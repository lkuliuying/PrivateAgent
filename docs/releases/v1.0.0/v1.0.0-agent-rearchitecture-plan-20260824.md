...
> 制定日期：2026-08-24  
> 计划状态：Approved for planning；实施前先完成 v0.9 不可变基线  
> 实施基线：`v0.9.0` 封版提交（待按封版清单生成）  
...
- 桌面默认使用 loopback WebSocket 或 Rust bridge，S1 spike 以安全性、背压、开发复杂度和打包稳定性决策最终 transport；
...
估算以 2 名全职工程师（Runtime/后端 1、桌面/协议 1）+ QA/安全兼职为基准；单人实施见 §21。
...
退出：v0.9 有可复现 commit；1.0 不携带未知工作区状态。
...
- 实作 JSON-RPC initialize + ping + bounded queue spike；
...
阈值可在 S1 基线测量后通过 ADR 调整，但不得在测试失败后临时放宽。
...
在 v0.9 工作区仍未收拢、版本源仍不一致或 Windows sandbox spike 尚未证明可行时，不开始大规模 v1.0 实现。
