...
class _HttpAdapter:
    ...
    async def _stream_sse(
        ...
        *,
        ...
        # 提前收到终止事件或取消时，逐层关闭解析器和 HTTP 行流。
        async with aclosing(self._stream_lines(url, headers=headers, payload=payload)) as lines:
            async with aclosing(iter_sse_events(lines, max_line_chars=self._MAX_SSE_LINE_CHARS,
                                                max_event_chars=self._MAX_SSE_EVENT_CHARS)) as events:
...
class OpenAIChatAdapter(_HttpAdapter):
    ...
    async def complete_stream(
        ...
        *,
        ...
        async with aclosing(self._stream_sse(
            f"{self.base_url}/chat/completions",
            headers=headers,
            payload=payload,
        )) as stream:
            async for _event_name, raw_data in stream:
                if cancellation.is_cancelled:
                    raise asyncio.CancelledError
                if raw_data == "[DONE]":
                    terminal = True
...
class OllamaChatAdapter(_HttpAdapter):
    ...
    async def complete_stream(
        ...
        *,
        ...
        async with aclosing(self._stream_lines(
            f"{self.base_url}/api/chat",
            headers={"Content-Type": "application/json"},
            payload=payload,
        )) as stream:
    ...
    async def complete_stream(
        ...
        *,
        ...
        async with aclosing(self._stream_sse(
            ...
            async for event_name, raw_data in stream:
                if cancellation.is_cancelled:
                    raise asyncio.CancelledError
                data = json.loads(raw_data)
                if not isinstance(data, dict):
