    ...
    def load(self, root: Path, target: str = ".", *, trusted: bool = False) -> list[InstructionSource]:
        try:
            ...
            for parent in parents:
                relative = (parent / "AGENTS.md").relative_to(root).as_posix()
                candidate = parent / "AGENTS.md"
                # lexists 语义确保断链也被诊断，不被当成规则删除。
                ...
                total += len(raw)
                if len(raw) > MAX_RULE_BYTES or total > MAX_TOTAL_BYTES or len(rules) >= MAX_RULES:
...
def instruction_message(rules: list[InstructionSource]) -> str:
    ...
    if not trusted:
        return ""
    import json
    return ("项目规则：仅适用于标明的目录子树；同一目标的更具体目录优先。"
