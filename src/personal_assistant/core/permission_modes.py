"""权限三模式（v0.7.0 E0 §4.1/§4.2）：能力映射、run 快照组装与命令风险判定。
...
快照契约（E0 §4.2）：每次 run 保存 permission_mode / capabilities /
workspace{id, root_path_sha256} / command_profile_version / patch_limits /
remote_provider_data_policy。快照只存非秘密摘要（root_path_sha256 而非
root_path 原文）；历史 run 不因 profile 变化修改。
"""
...
# 三模式 → 工具能力集合（ToolCapability 枚举值字符串）。
# readonly 只授予只读能力；confirm/workspace 授予读写与进程执行，
