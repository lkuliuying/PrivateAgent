...
from private_agent_core.llm.url_policy import (
...
@pytest.mark.asyncio
async def test_message_stream_never_retries_after_public_commentary():
    class Adapter:
        provider_name = "message-stream"
        model_name = "fixture"
        capabilities = ModelCapabilities(True, False, False, False, True)
        calls = 0

        async def complete_stream(self, request, *, cancellation, on_delta):
            raise AssertionError("应选择消息流协议")

        async def complete_stream_messages(self, request, *, cancellation, on_delta, on_message_delta):
            self.calls += 1
            await on_message_delta(ModelTextDelta("progress", "commentary", "正在检查"))
            raise httpx.ConnectError("fixture interruption")

    adapter, messages, legacy = Adapter(), [], []
    ...
    with pytest.raises(ModelGatewayError, match="network_error"):
        await ModelGateway(adapter, retry_policy=RetryPolicy(max_attempts=2, initial_backoff_seconds=0)).complete_stream(
            ModelRequest(messages=(ModelMessage(role="user", content="检查"),)), cancellation=CancellationToken(),
            on_delta=receive, on_message_delta=receive_message)
    assert adapter.calls == 1 and legacy == []
    assert messages == [ModelTextDelta("progress", "commentary", "正在检查")]
